...
class _EditVendorScreenState extends State<EditVendorScreen> {
  ...
  void initState() {
    super.initState();
    // Prefill the form with existing vendor data
  ...
  Future<void> _loadCategories() async {
    try {
      ...
      // Load existing categories to check for uniqueness (excluding current vendor)
      final weddingEvent = await _databaseHelper.getWeddingEvent();
      if (weddingEvent != null) {
        final vendors = await _databaseHelper.getVendorsByWeddingEvent(weddingEvent.id!);
        setState(() {
          _availableCategories = categories;
          // Exclude current vendor's category from existing categories
          _existingCategories = vendors
              .where((vendor) => vendor.id != widget.vendor.id)
  ...
  Future<void> _updateVendor() async {
    if (_formKey.currentState!.validate() && _selectedCategory != null) {
      // Check if category already exists (only if changing category)
      if (_selectedCategory != widget.vendor.category && _existingCategories.contains(_selectedCategory)) {
      ...
      try {
        ...
        await _databaseHelper.updateVendor(updatedVendor);
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: _availableCategories.map((String category) {
                  ...
                  final isCurrentCategory = category == widget.vendor.category;
                  return DropdownMenuItem<String>(
                    value: category,
                    enabled: !isUsed || isCurrentCategory,
                    child: Row(
                      children: [
                        Text(
                          category,
                          style: TextStyle(
                            color: (isUsed && !isCurrentCategory) ? Colors.grey : Colors.white,
                          ),
                        ),
                        if (isUsed && !isCurrentCategory) ...[
